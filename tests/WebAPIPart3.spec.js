...
test.beforeAll( async({browser}) => {
    ...
    await context.storageState({path: 'state.json'}); //Captures browser data in a json file called state.json (Check)
    webContext = await browser.newContext({storageState: 'state.json'}) // pass the state.json file into the variable context created
})

// Based on the above, we can skip login for all test cases defined below:

test('Test case 1', async() => {
    const productName = "ADIDAS ORIGINAL";

    const page =  await webContext.newPage();
//Opens a new page based on the Browser details received from state.json in before all and bypasses login to perform remaining actions
    await page.goto("https://rahulshettyacademy.com/client");
   ...
   // await page.locator("[placeholder='Select Country']").pressSequentially('Gha'); //To type sequentially
    ...
    await expect(page.locator(".user__name [type*='text']").first()).toHaveText(primaryemail);
